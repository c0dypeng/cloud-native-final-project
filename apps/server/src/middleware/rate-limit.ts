...
/** No-op middleware used in NODE_ENV=test so suites don't trip the limiter. */
const passthrough: RequestHandler = (_req, _res, next) => next();

export const loginLimiter: RequestHandler = IS_TEST
  ? passthrough
  : rateLimit({
      windowMs: 60 * 1000,
      limit: 5,
...
export const adminLoginLimiter: RequestHandler = IS_TEST
  ? passthrough
  : rateLimit({
      windowMs: 60 * 1000,
      limit: 3,
 ...
 */
export const reportLimiter: RequestHandler = IS_TEST
  ? passthrough
  : rateLimit({
      windowMs: 60 * 1000,
      limit: 30,
